...
export interface InvokeCeremonyContext<TStartResult, TFinishResult, TCeremonyOptions extends CredentialCreationOptions | CredentialRequestOptions> {
    ...
    finish: (startResult: TStartResult, credential: PublicKeyCredential) => Promise<Result<TFinishResult, TryMellonError>>;
}
 ...
 */
export async function invokeCeremony<TStartResult, TFinishResult, TCeremonyOptions extends CredentialCreationOptions | CredentialRequestOptions>(
    ...
    try {
        ...
        if (!credential) {
            const error = createInvalidArgumentError('credential', `${operation === 'register' ? 'creation' : 'retrieval'} failed`);
            eventEmitter.emit('error', { type: 'error', error });
            return err(error);
        }
